...
import com.skillhub.confettiOnFinishSession
import com.skillhub.globalModules
import com.skillhub.modules.ModuleTask
import com.skillhub.modules.TaskFragment
import com.skillhub.modules.TaskState
...
class SessionManager(sessionId: Int, private val activity: SessionViewer) {
    ...
    private fun makeTask(): ModuleTask {
        val rand = (0 until currentSession.getConfigs().size).random()
        val config = currentSession.getConfigs().elementAt(rand)
        val moduleID = database.loadModuleIDFromConfigID(config.getConfigID())
        val module = globalModules[moduleID]!!
        val taskID = database.makeNewTaskID()
        val taskConfig = database.loadConfig(config.getConfigID())
        val task = module.makeTask(taskID, taskConfig, currentSession)
    ...
    fun getTaskByPosition(position: Int): ModuleTask {
        return database.loadTasks(currentSession)[position]
    }
...
//        }
        ...
        pager.registerOnPageChangeCallback(object : ViewPager2.OnPageChangeCallback() {
            override fun onPageSelected(position: Int) {
                ...
                supportActionBar?.title = getCurrentTask().getModule().getStub().descriptionName
