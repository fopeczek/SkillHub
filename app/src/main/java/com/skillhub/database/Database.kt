package com.skillhub.database

import com.skillhub.ColumnType
import com.skillhub.sessions.Session
import com.skillhub.getTimestamp
...
class Database(private val queryHelper: QueryHelper) {
    ...
    fun saveSession(session: Session) {
        val data = mapOf(
            ...
            configIDs to session.getConfigs().joinToString(",", transform = { it.getConfigID().toString() }),
    ...
    fun removeSession(session: Session) {
        queryHelper.removeRow(sessionsTable, sessionID, session.getSessionID())
        val tasks = loadTasks(session)
        for (task in tasks) {
    ...
    private fun fillSessionFromMap(data: Map<String, Any>): Session {
        val tasks = loadTasks(fillSessionFromMap(data, mutableListOf()))// using sessionID causes infinite recursion
        return fillSessionFromMap(data, tasks)
    }

    private fun fillSessionFromMap(data: Map<String, Any>, tasks: MutableList<ModuleTask>): Session {
        val sessionID = data[sessionID] as Int
        val sessionName = data[sessionName] as String
        val configs = (data[configIDs] as String).split(",").map { loadConfig(it.toInt()) }.toMutableList()
        val points = data[points] as Int
        val targetPoints = data[targetPoints] as Int
        val reset = (data[reset] as Int) == 1
        val pointPenalty = data[penaltyPoints] as Int
        val answeredTaskAmount = loadAmountOfAttemptedTasks(sessionID)
        return Session(
            sessionID, this, configs, tasks, sessionName, reset, pointPenalty, targetPoints, answeredTaskAmount, points
        )
    }
    ...
    fun saveConfig(config: ModuleConfig) {
        val data = mapOf(
            configID to config.getConfigID(),
            configName to config.getName(),
            moduleID to config.getModule().getModuleID()
        )
    ...
    fun loadModuleIDFromConfigID(configID: Int): Int {
        val data = queryHelper.getRow(configsTable, Companion.configID, configID)
    ...
    private fun fillConfigFromMap(data: Map<String, Any>): ModuleConfig {
        ...
        val module = globalModules[data[moduleID] as Int]!!
        val configData = loadConfigData(configID)
        return ModuleConfig(configID, module, this, configName, configData)
    }
    ...
    fun saveTask(task: ModuleTask) {
        val data = mapOf(
            taskID to task.getTaskID(),
            moduleID to task.getModule().getModuleID(),
            sessionID to task.getSession().getSessionID(),
            configID to task.getConfig().getConfigID(),
            question to task.getQuestion().getQuestion(),
            timestamp to getTimestamp()
        )
    ...
    fun loadTasks(session: Session): MutableList<ModuleTask> {
        val data = queryHelper.getAllFiltered(taskTable, sessionID, session.getSessionID(), true)
        val tasks = mutableListOf<ModuleTask>()
        for (row in data) {
            tasks.add(fillTaskFromMap(row, session))
        }
        return tasks
    }
    ...
    private fun fillTaskFromMap(data: Map<String, Any>): ModuleTask {
        val session = loadSession(data[sessionID] as Int)
        return fillTaskFromMap(data, session)
    }

    private fun fillTaskFromMap(data: Map<String, Any>, session: Session): ModuleTask {
        ...
        return module!!.deserializeTask(question, answers, attempts, taskID, config, session)
    }
    ...
    fun saveAnswer(answerID: Int, taskID: Int, answer: String) {
        val data = mapOf(
            Companion.answerID to answerID,
            Companion.taskID to taskID,
            Companion.answer to answer,
            timestamp to getTimestamp()
        )
