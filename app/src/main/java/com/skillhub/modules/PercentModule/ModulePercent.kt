package com.skillhub.modules.PercentModule
...
class PercentModule(moduleID: Int, stub: ModuleStub) : Module(moduleID, stub,
    { module, session, question, answers, taskID, config, attempt -> PercentTask(module, session, question, answers, taskID, config, attempt) },
    ...
    override fun makeTask(taskID: Int, config: ModuleConfig, session: Session): ModuleTask {
        ...
        val question = "$value%"
        return PercentTask(this, session, PercentQuestion(question), listOf(PercentAnswer(0, value)), taskID, config)
    }
...
) : ModuleTask(module, session, question, answers, taskID, config, loadedAttemptID)
